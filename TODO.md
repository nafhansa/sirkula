...
- [ ] Inisialisasi repo monorepo (Next.js atau Vite + React)
...
- [ ] Buat file `tokens.css` atau `globals.css` dengan semua CSS custom properties:
  ...
  - [ ] `--color-danger: #F44336`
  - [ ] `--color-info: #2196F3`
  - [ ] `--color-neutral-100` hingga `--color-neutral-900` (6 level)
  - [ ] `--color-plastic: #2196F3`
  - [ ] `--color-paper: #FF9800`
  - [ ] `--color-residue: #795548`
- [ ] Buat typography tokens:
  - [ ] Font stack sans & mono
  - [ ] Font sizes mobile: `xs` (12px) s/d `4xl` (32px)
  - [ ] Font sizes admin: `xs` (11px) s/d `xl` (16px)
  - [ ] Font weights: `light`, `regular`, `semibold`, `bold`
- [ ] Buat spacing tokens: `xs` (4px), `sm` (8px), `md` (16px), `lg` (24px), `xl` (32px), `2xl` (48px)
- [ ] Buat border radius tokens: `sm` (4px), `md` (8px), `lg` (12px), `xl` (16px), `full` (9999px)
- [ ] Buat border tokens: `--border-light`, `--border-default`
- [ ] Buat shadow tokens: student app (`sm`, `md`, `lg`) dan admin (`admin-sm`, `admin-md`, `admin-lg`)
- [ ] Setup Tailwind config dengan semua token di atas (jika pakai Tailwind)

### 0.3 Setup Dependencies
- [ ] Install `html5-qrcode` atau library QR scanner
- [ ] Install Cloudinary SDK untuk photo upload
- [ ] Install charting library (Recharts / Chart.js / Nivo)
- [ ] Install form validation library (`zod` + `react-hook-form`)
- [ ] Install state management (Zustand / Redux Toolkit)
- [ ] Install WebSocket client library
- [ ] Install IndexedDB wrapper (idb / Dexie.js) untuk offline support
...
- [ ] Button press: scale 95% (50ms), instant release
  ```css
  @keyframes buttonPress { 0% {scale:1} 50% {scale:0.95} 100% {scale:1} }
  button:active { animation: buttonPress 50ms ease-out; }
  ```
...
*Based on: README.md v1.0 – UI/UX Design System & Component Specifications*
